...
file_path <- "/mnt/data/fbraddock/familial_kc/indel/"

# define the path where files will be saved
export.loc <- "/mnt/data/fbraddock/familial_kc/indel/results/"
...
for (family_name in unique(fam.dt$family)) {
  ...
  for (individual in unique(fam.dt[family == family_name & status != "unaffected", individual])) {
    individual_dt <- fread(paste0(file_path, individual, "/", individual, ".GATK.indel.annovar.hg38_multianno.xls"))
  ...
  DFM5 <- DFM4[DFM4[ExonicFunc != "frameshift deletion"]]
  # filter for synonymous 
  DFM6 <- DFM4[DFM4[ExonicFunc == "frameshift deletion"]]
  ...
  #split and Filter non-synonymous
  DFM11 <- DFM10[DFM10[ExonicFunc != "frameshift deletion"]]
  # filter for synonymous 
  DFM12 <- DFM10[DFM10[ExonicFunc == "frameshift deletion"]]
  #Filter intronic variants
  ...
  file.list<- c("het_rare", "het_exonic", "het_frameshift_deletion", "het_non_frameshift_deletion", "het_intronic", "het_intergenic&other", "homo_rare", "homo_exonic", "homo_frameshift_deletion", "homo_non_frameshift_deletion", "homo_intronic", "homo_intergenic&other", "het_homo_exonic")

  for(n in number.list){
    print(n)
    write.csv(dfm.list[[n]], file=paste0(export.loc, family_name, "/", family_name,"_", file.list[[n]], "_indel.csv"), row.names=FALSE)
