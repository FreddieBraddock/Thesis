...
for (family_name in unique(fam.dt$family)) {
  ...
  for (individual in unique(fam.dt[family == family_name & status == "unaffected", individual])) {
    ...
    #Do I just change al values to numeric and then NA to 0??
    ...
    family_number <- data.frame(family_name = c(nrow(DFM11), nrow(DFM12), nrow(DFM13), nrow(DFM14), nrow(DFM5), nrow(DFM6), nrow(DFM7), nrow(DFM8)))

    number_variants_dt<- rbind(number_variants_dt, family.number)
